...
function themify_builder_init() {
	...
	if ( class_exists( 'Themify_Builder') ) {
		...
		// Flush rewrite
		themify_builder_maybe_rewrite_rules();
	} // class_exists check
}
 ...
 */
function themify_builder_maybe_rewrite_rules() {
	$ver = THEMIFY_VERSION; // Get the version number
	$defaults = array( 'version' => 0 );
	$r = wp_parse_args( get_option( __CLASS__ . '_flush', array() ), $defaults );
	if ( $r['version'] != $ver ) { // Flush if ver changes.
		flush_rewrite_rules();
		$args = array( 'version' => $ver );
