	...
	private function cg_get_image($single){
		...
		if(get_cg_option('image_source')=='featured'){
			if (has_post_thumbnail($single->ID )){
				$image = wp_get_attachment_image_src(get_post_thumbnail_id( $single->ID ), 'thumbnail' );
				$cg_img = $image[0];
			}
			else {
		...
		return '<a href='.$returnlink.'><img src="'.$cg_img.'" alt="'.$single->post_title.'" title="'.$single->post_title.'"/></a>';
